...
const prefix = 'trafficViolation'
const urlPrefix = '/driver/trafficViolation'
...
export default extend({
  ...
  reducers: {
    ...
    // 修改页面
    toEdit(state, action) {
      return { ...state, register: true, res: action.res, trafficViolation: action.trafficViolation }
    },
  ...
  effects: {
    ...
    * init({}, { update, tableBindType, formBindType, select }) {
      const { init } = yield select(({ trafficViolationStore }) => trafficViolationStore)
      if (!init) {
        yield tableBindType({
        })
