...
const prefix = 'punishtrafficAccident'
const urlPrefix = '/driver/punishtrafficAccident'
...
export default extend({
  ...
  state: {
    ...
    // 条件查询 -- carNo and plateNumber，documentNo
    carNo: [],
    plateNumber: [],
    documentNo: [],
  ...
  reducers: {
    ...
    // 修改页面
    toEdit(state, action) {
      return { ...state, register: true, res: action.res, trafficAccident: action.trafficAccident }
    },
  ...
  effects: {
    ...
    * init({}, { update, tableBindType, formBindType, select }) {
      const { init } = yield select(({ punishtrafficAccidentStore }) => punishtrafficAccidentStore)
      if (!init) {
        yield tableBindType({
        })
