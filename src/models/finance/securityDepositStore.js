...
export default extend({
  ...
  reducers: {
    ...
    // 修改页面
    toEdit(state, action) {
      return { ...state, register: true, res: action.res, securityDeposit: action.securityDeposit }
    },
  ...
  effects: {
    ...
    * init({}, { update, tableBindType, formBindType, select }) {
      const { init } = yield select(({ securityDepositStore }) => securityDepositStore)
      if (!init) {
        yield tableBindType({
        })
